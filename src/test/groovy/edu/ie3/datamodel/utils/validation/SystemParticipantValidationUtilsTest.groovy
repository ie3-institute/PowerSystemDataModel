...
class SystemParticipantValidationUtilsTest extends Specification {
	...
	def "SystemParticipantValidationUtils.checkPV() recognizes all potential errors for a PV"() {
		...
		SystemParticipantTestData.pvInput.copy().albedo(2).build() || new InvalidEntityException("Albedo of the plant's surrounding of the PV unit must be between 0 and 1", invalidPV)
		SystemParticipantTestData.pvInput.copy().azimuth(Quantities.getQuantity(-100d, AZIMUTH)).build() || new InvalidEntityException("Azimuth angle of the PV unit must be between -90° (east) and 90° (west)", invalidPV)
		SystemParticipantTestData.pvInput.copy().etaConv(Quantities.getQuantity(110d, EFFICIENCY)).build() || new InvalidEntityException("Efficiency of the converter of the PV unit must be between 0% and 100%", invalidPV)
		SystemParticipantTestData.pvInput.copy().height(Quantities.getQuantity(100d, SOLAR_HEIGHT)).build() || new InvalidEntityException("Tilted inclination from horizontal of the PV unit must be between 0° and 90°", invalidPV)
