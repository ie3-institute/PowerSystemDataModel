...
class SystemParticipantValidationUtilsTest extends Specification {
  ...
  def "SystemParticipantValidationUtils.checkEvType() recognizes all potential errors for an EV type"() {
    ...
    new EvTypeInput(uuid, id, capex, opex, Quantities.getQuantity(0, ENERGY_IN), Quantities.getQuantity(0, ENERGY_PER_DISTANCE), Quantities.getQuantity(25, S_RATED), cosPhiRated, Quantities.getQuantity(20, ACTIVE_POWER_IN)) || new InvalidEntityException("The following quantities have to be positive: 0 kWh, 0 kWh/km", invalidEvType)
  }
