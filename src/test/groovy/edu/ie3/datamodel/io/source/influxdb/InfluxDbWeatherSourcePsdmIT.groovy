...
class InfluxDbWeatherSourcePsdmIT extends Specification implements WeatherSourceTestHelper {
	...
	def setupSpec() {
		...
		source = new InfluxDbWeatherSource(connector, PsdmWeatherTestData.coordinateSource, weatherFactory)
	}
	...
	def "An InfluxDbWeatherSource can read multiple time series values for multiple coordinates"() {
		...
		equalsIgnoreUUID(coordinateToTimeSeries.get(PsdmWeatherTestData.COORDINATE_193186), timeseries_193186)
		equalsIgnoreUUID(coordinateToTimeSeries.get(PsdmWeatherTestData.COORDINATE_193187), timeseries_193187)
	}
	...
	def "An InfluxDbWeatherSource can read all weather data in a given time interval"() {
		...
		equalsIgnoreUUID(coordinateToTimeSeries.get(PsdmWeatherTestData.COORDINATE_193186).getEntries(), timeseries_193186.getEntries())
		equalsIgnoreUUID(coordinateToTimeSeries.get(PsdmWeatherTestData.COORDINATE_193187).getEntries(), timeseries_193187.getEntries())
		equalsIgnoreUUID(coordinateToTimeSeries.get(PsdmWeatherTestData.COORDINATE_193188).getEntries(), timeseries_193188.getEntries())
	}

	def "An InfluxDbWeatherSource will return an equivalent to 'empty' when being unable to map a coordinate to it's ID"() {
		def validCoordinate = PsdmWeatherTestData.COORDINATE_193186
		def invalidCoordinate = GeoUtils.xyToPoint(48d, 7d)
		def time = PsdmWeatherTestData.TIME_15H
		def timeInterval = new ClosedInterval(PsdmWeatherTestData.TIME_15H, PsdmWeatherTestData.TIME_17H)
