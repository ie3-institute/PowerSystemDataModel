...
class CsvFileSinkTest extends Specification implements TimeSeriesTestData {
	...
	def "The StringUtils converts a given Array of csv header elements to match the csv specification RFC 4180 "() {
		...
		when:
		def actual = input.stream().map({ inputElement -> CsvFileSink.csvString(inputElement, ",") }).collect(Collectors.toSet()) as Set

		then:
		actual == expected
	}

	/*Will move to PowerSystemDataUtils*/
	...
	def "The StringUtils converts a given LinkedHashMap of csv data to match the csv specification RFC 4180 "() {
		...
		def actualList = input.entrySet().stream().map({ mapEntry ->
			return new AbstractMap.SimpleEntry<String, String>(CsvFileSink.csvString(mapEntry.key, ","), CsvFileSink.csvString(mapEntry.value, ","))
		}) as Set
	...
	def "The StringUtils converts a given Array of csv header elements to match the csv specification RFC "() {
		expect:
		CsvFileSink.csvString(inputString, csvSep) == expect
