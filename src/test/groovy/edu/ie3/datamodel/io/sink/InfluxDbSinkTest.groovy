...
@Testcontainers
class InfluxDbSinkTest extends Specification {
    ...
    def "An InfluxDbSink can persist a ResultEntity"() {
        ...
        def key = fileNamingStrategy.getFileName(LineResult.class).get()
    ...
    def "An InfluxDbSink can persist multiple different ResultEntities"() {
        ...
        def entities = [lineResult1, lineResult2, lineResult3, chpResult1, chpResult2]
        when:
        sink.persistAll(entities)
        def key_line = fileNamingStrategy.getFileName(LineResult.class).get()
        def key_chp = fileNamingStrategy.getFileName(ChpResult.class).get()
    ...
    def "An InfluxDbSink can persist a TimeSeries"() {
        given:
        def p1 = new TimeBasedValue(ZonedDateTime.of(2020, 5, 3, 14, 18, 0, 0, ZoneId.of(("UTC"))),
                new PValue(Quantities.getQuantity(5d, StandardUnits.ACTIVE_POWER_IN)))
        def p2 = new TimeBasedValue(ZonedDateTime.of(2020, 5, 3, 14, 18, 30, 0, ZoneId.of(("UTC"))),
                new PValue(Quantities.getQuantity(10d, StandardUnits.ACTIVE_POWER_IN)))
        def p3 = new TimeBasedValue(ZonedDateTime.of(2020, 5, 3, 14, 19, 0, 0, ZoneId.of(("UTC"))),
                new PValue(Quantities.getQuantity(-5d, StandardUnits.ACTIVE_POWER_IN)))
        def timeSeries = new IndividualTimeSeries(UUID.randomUUID(), [p1, p2, p3] as Set<TimeBasedValue>)
        when:
        sink.persist(timeSeries)
        def key = fileNamingStrategy.getFileName(timeSeries).get()
        def queryResult = connector.getSession().query(new Query("SELECT * FROM " + key))
        def parsedResults = InfluxDbConnector.parseQueryResult(queryResult)
