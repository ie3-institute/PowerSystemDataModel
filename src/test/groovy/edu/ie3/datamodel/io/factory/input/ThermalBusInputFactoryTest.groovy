...
class ThermalBusInputFactoryTest extends Specification implements FactoryTestHelper {
  ...
  def "A ThermalBusInputFactory should parse a valid SwitchInput correctly"() {
    ...
    when:
    Try<ThermalBusInput, FactoryException> input = inputFactory.get(new AssetInputEntityData(new FactoryData.MapWithRowIndex("-1", parameter), inputClass, operatorInput))
