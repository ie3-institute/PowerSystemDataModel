...
class FixedFeedInInputFactoryTest extends Specification implements FactoryTestHelper {
  ...
  def "A FixedFeedInInputFactory should parse a valid FixedFeedInInput correctly"() {
    ...
    when:
    Try<FixedFeedInInput> input = inputFactory.get(new NodeAssetInputEntityData(new FactoryData.MapWithRowIndex("-1", parameter), inputClass, operatorInput, nodeInput))
  ...
  def "A FixedFeedInInputFactory should throw an exception on invalid or incomplete data (parameter missing)"() {
    ...
    when:
    Try<FixedFeedInInput> input =  inputFactory.get(new NodeAssetInputEntityData(new FactoryData.MapWithRowIndex("-1", parameter), inputClass, nodeInput))
