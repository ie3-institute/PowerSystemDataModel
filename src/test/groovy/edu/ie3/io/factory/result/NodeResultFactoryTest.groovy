...
class NodeResultFactoryTest extends Specification {
    ...
    def "A NodeResultFactory should parse a WecResult correctly"() {
        ...
        result.get().vMag == Quantities.getQuantity(Double.parseDouble(parameterMap.get("vmag")), StandardUnits.ELECTRIC_VOLTAGE_MAGNITUDE)
        result.get().vAng == Quantities.getQuantity(Double.parseDouble(parameterMap.get("vang")), StandardUnits.ELECTRIC_VOLTAGE_ANGLE)
