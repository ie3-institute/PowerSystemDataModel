...
public class WeatherTestEntityBuilder {
  ...
  public static final ZonedDateTime MIDDLE_DATE =
      ZonedDateTime.of(2013, 4, 3, 12, 0, 0, 0, ZoneId.of("UTC"));
  public static final ZonedDateTime END_DATE =
      ZonedDateTime.of(2013, 4, 7, 23, 59, 0, 0, ZoneId.of("UTC"));
  public static final ZonedDateTime INVALID_DATE =
      ZonedDateTime.of(1995, 1, 1, 0, 0, 0, 0, ZoneId.of("UTC"));
  public static final ClosedInterval<ZonedDateTime> MONTH_INTERVAL =
  ...
  public static final List<Point> TEST_COORDINATES =
      Arrays.asList(TEST_COORDINATE_A, TEST_COORDINATE_B, TEST_COORDINATE_C);
  ...
  public static void fillWeatherValues() {
    ...
    try {
      String file =
          WeatherTestEntityBuilder.class.getClassLoader().getResource("weather.csv").getFile();
      csvWeatherInputs =
          new CsvToBeanBuilder(new FileReader(file))
              .withType(CsvWeatherInput.class)
              .withSeparator(';')
              .build()
              .parse();
    ...
    List<TimeBasedValue<WeatherValues>> weatherValuesWithoutUuid =
        csvWeatherInputs.stream()
            .map(csvInput -> csvInput.toTimeBasedWeatherValues())
            .collect(Collectors.toList());
    weatherValues =
        weatherValuesWithoutUuid.stream()
            .map(
                tbv ->
                    new TimeBasedValue<WeatherValues>(
                        UNSPECIFIC_UUID, tbv.getTime(), tbv.getValue()))
            .collect(Collectors.toList());
  }
}
