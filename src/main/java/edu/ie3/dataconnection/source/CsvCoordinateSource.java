...
public class CsvCoordinateSource { //TODO replace with real Csv Reader
    ...
    public static void fillCoordinateMaps(){
        ...
        try {
            ...
            String file = CsvCoordinateSource.class.getClassLoader().getResource("eu_coords.csv").getFile();
            reader = new CSVReaderBuilder(new FileReader(file)).withCSVParser(parser)
                    .build();
            ...
            while ((nextLine = reader.readNext()) != null) {
                idToCoordinate.put(Integer.parseInt(nextLine[idIndex]), CoordinateTools.xyCoordToPoint(Double.parseDouble( nextLine[latIndex]), Double.parseDouble( nextLine[lonIndex])));
            }
