...
public class ValidationUtils {
    ...
    public static void checkGrid(GridContainer gridContainer) {
        ...
        checkGraphicElements(gridContainer.getGraphics(), gridContainer.getRawGrid().getNodes(),
     ...
     */
    public static void checkSystemParticipants(SystemParticipants systemParticipants, Set<NodeInput> nodes) {
     ...
     */
    public static void checkGraphicElements(GraphicElements graphicElements,
                                            Set<NodeInput> nodes,
                                            Set<LineInput> lines) {
        if(graphicElements == null)
            throw new NullPointerException("Expected graphic elements, but got nothing. :-(");

        graphicElements.getNodeGraphics().forEach(graphic -> {
            if(!nodes.contains(graphic.getNode()))
                throw new InvalidEntityException(
                                "The node graphic refers to a node, that is not among the provided ones.", graphic);
        });

        graphicElements.getLineGraphics().forEach(graphic -> {
            if(!lines.contains(graphic.getLine()))
                throw new InvalidEntityException(
                                "The line graphic refers to a line, that is not among the provided ones.", graphic);
        });
    }
     ...
     * @param nodes       Permissible, already known nodes
     */
    private static void checkNodeAvailability(Transformer3WInput transformer, Collection<NodeInput> nodes) {
        if(!nodes.contains(transformer.getNodeA()) || !nodes.contains(transformer.getNodeB()) ||
     ...
     * @param nodes       Permissible, already known nodes
     */
    private static void checkNodeAvailability(SystemParticipantInput participant, Collection<NodeInput> nodes) {
        if(!nodes.contains(participant.getNode()))
            throw getMissingNodeException(participant);
    }
     ...
     * @param nodes           Permissible, already known nodes
     */
    private static void checkNodeAvailability(MeasurementUnitInput measurementUnit, Collection<NodeInput> nodes) {
        if(!nodes.contains(measurementUnit.getNode()))
            throw getMissingNodeException(measurementUnit);
    }
    ...
    public static void checkLineType(LineTypeInput lineType) {
        ...
        if(lineType.getvRated() == null || lineType.getiMax() == null || lineType.getB() == null ||
           lineType.getX() == null || lineType.getR() == null || lineType.getG() == null)
        ...
        detectZeroOrNegativeQuantities(new Quantity<?>[] { lineType.getvRated(), lineType.getiMax(), lineType.getX(),
                                                           lineType.getR() }, lineType);
    }
    ...
    public static void checkTransformer2WType(Transformer2WTypeInput trafoType) {
        ...
        if((trafoType.getsRated() == null) || (trafoType.getvRatedA() == null) || (trafoType.getvRatedB() == null) ||
           (trafoType.getrSc() == null) || (trafoType.getxSc() == null) || (trafoType.getgM() == null) ||
           (trafoType.getbM() == null) || (trafoType.getdV() == null) || (trafoType.getdPhi() == null))
            throw new InvalidEntityException("at least one value of trafo2w type is null", trafoType);

        detectNegativeQuantities(new Quantity<?>[] { trafoType.getgM(), trafoType.getbM(), trafoType.getdPhi() },
                        trafoType);
        detectZeroOrNegativeQuantities(
                        new Quantity<?>[] { trafoType.getsRated(), trafoType.getvRatedA(), trafoType.getvRatedB(),
                                            trafoType.getxSc(), trafoType.getdV() }, trafoType);
    }
    ...
    public static void checkTransformer3WType(Transformer3WTypeInput trafoType) {
        ...
        if((trafoType.getsRatedA() == null) || (trafoType.getsRatedB() == null) || (trafoType.getsRatedC() == null) ||
           (trafoType.getvRatedA() == null) || (trafoType.getvRatedB() == null) || (trafoType.getvRatedC() == null) ||
           (trafoType.getrScA() == null) || (trafoType.getrScB() == null) || (trafoType.getrScC() == null) ||
           (trafoType.getxScA() == null) || (trafoType.getxScB() == null) || (trafoType.getxScC() == null) ||
           (trafoType.getgM() == null) || (trafoType.getbM() == null) || (trafoType.getdV() == null) ||
    ...
    private static InvalidGridException getMissingNodeException(AssetInput input) {
        return new InvalidGridException(input.getClass().getSimpleName() + " " + input +
    ...
    private static void detectNegativeQuantities(Quantity<?>[] quantities, UniqueEntity entity) {
        ...
        detectMalformedQuantities(quantities, entity, predicate,
     ...
     * @param entity     Unique entity holding the malformed quantities
     */
    private static void detectZeroOrNegativeQuantities(Quantity<?>[] quantities, UniqueEntity entity) {
        Predicate<Quantity<?>> predicate = quantity -> quantity.getValue().doubleValue() <= 0;
        detectMalformedQuantities(quantities, entity, predicate, "The following quantities have to be positive");
    }
     ...
     */
    private static void detectMalformedQuantities(Quantity<?>[] quantities,
                                                  UniqueEntity entity,
                                                  Predicate<Quantity<?>> predicate,
                                                  String msg) {
        String malformedQuantities = Arrays.stream(quantities).filter(predicate).map(Quantity::toString)
    ...
    public static boolean distinctUuids(Set<? extends UniqueEntity> entities) {
        return entities.stream().filter(distinctByKey(UniqueEntity::getUuid)).collect(Collectors.toSet()).size() ==
               entities.size();
    }
    ...
    public static Optional<String> checkForDuplicateUuids(Set<UniqueEntity> entities) {
        ...
        String duplicationsString =
                        entities.stream().collect(Collectors.groupingBy(UniqueEntity::getUuid, Collectors.counting()))
                                        .entrySet().stream().filter(entry -> entry.getValue() > 1).map(entry -> {
                            String duplicateEntitiesString =
                                            entities.stream().filter(entity -> entity.getUuid().equals(entry.getKey()))
