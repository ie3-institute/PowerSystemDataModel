...
public enum BdewLoadProfile implements StandardLoadProfile {
    ...
    public static BdewLoadProfile get(String key) {
        return Arrays.stream(BdewLoadProfile.values()).filter(loadProfile -> loadProfile.key.equalsIgnoreCase(key))
                        .findFirst().orElseThrow(() -> new IllegalArgumentException(
                                        "No predefined bdew load profile with key '" + key +
                                        "' found. Please provide one of the following keys:" +
                                        Arrays.stream(BdewLoadProfile.values()).map(BdewLoadProfile::getKey)
