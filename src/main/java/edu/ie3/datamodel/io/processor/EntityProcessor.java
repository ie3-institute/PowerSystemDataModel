...
public abstract class EntityProcessor<T extends UniqueEntity> {
    ...
    private String[] registerClass(Class<? extends T> cls, List<Class<? extends T>> eligibleClasses) {
        if(!eligibleClasses.contains(cls))
            throw new EntityProcessorException(
                            "Cannot register class '" + cls.getSimpleName() + "' with entity processor '" +
                            this.getClass().getSimpleName() + "'. Eligible classes: " +
                            eligibleClasses.stream().map(Class::getSimpleName).collect(Collectors.joining(",")));
        try {
        ...
        String[] filteredArray = fieldNameToMethod.keySet().stream().filter(x -> !x.toLowerCase().contains(uuidString))
                        .toArray(String[]::new);
    ...
    public Optional<LinkedHashMap<String, String>> handleEntity(T entity) {
        if(!registeredClass.equals(entity.getClass()))
            throw new EntityProcessorException("Cannot process " + entity.getClass().getSimpleName() +
                                       ".class with this EntityProcessor. Please either provide an element of " +
                                       registeredClass.getSimpleName() + ".class or create a new factory for " +
    ...
    protected Optional<String> handleQuantity(Quantity<?> quantity, String fieldName) {
        ...
        case "iBMag":
        case "iCMag":
            normalizedQuantityValue = quantityValToOptionalString(
                            quantity.asType(ElectricCurrent.class).to(StandardUnits.ELECTRIC_CURRENT_MAGNITUDE));
            break;
        case "qDot":
            normalizedQuantityValue =
                            quantityValToOptionalString(quantity.asType(Power.class).to(StandardUnits.Q_DOT_RESULT));
            break;
        case "fillLevel":
            normalizedQuantityValue = quantityValToOptionalString(
            ...
            break;
        default:
            log.error("Cannot process quantity {} for field with name {} in model processing!", quantity, fieldName);
            break;
        }
     ...
     * or empty if an error occurred during processing
     */
    protected abstract Optional<String> handleProcessorSpecificQuantity(Quantity<?> quantity, String fieldName);
