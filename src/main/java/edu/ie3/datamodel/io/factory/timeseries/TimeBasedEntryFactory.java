...
public class TimeBasedEntryFactory extends EntityFactory<TimeBasedValue, TimeBasedEntryData> {
    ...
    private static final TimeUtil timeUtil = new TimeUtil(ZoneId.of("UTC"), Locale.GERMANY, "yyyy-MM-dd'T'HH:mm:ss'Z'");
    ...
    protected List<Set<String>> getFields(TimeBasedEntryData data) {
        ...
        if(valueClass.equals(WeatherValue.class)){
            minConstructorParams = newSet(TIME, COORDINATE, DIFFUSE_IRRADIATION, DIRECT_IRRADIATION, TEMPERATURE, WIND_DIRECTION, WIND_VELOCITY);
        }
    ...
    protected TimeBasedValue buildModel(TimeBasedEntryData data) {
        ...
        if(valueClass.equals(WeatherValue.class)){
            ...
            ComparableQuantity<Irradiation> directIrradiation = data.getQuantity(DIRECT_IRRADIATION, StandardUnits.IRRADIATION);
            ComparableQuantity<Irradiation> diffuseIrradiation = data.getQuantity(DIFFUSE_IRRADIATION, StandardUnits.IRRADIATION);
            ComparableQuantity<Temperature> temperature = data.getQuantity(TEMPERATURE, StandardUnits.TEMPERATURE);
            ComparableQuantity<Angle> windDirection = data.getQuantity(WIND_DIRECTION, StandardUnits.WIND_DIRECTION);
            ComparableQuantity<Speed> windVelocity = data.getQuantity(WIND_VELOCITY, StandardUnits.WIND_VELOCITY);
            WeatherValue weatherValue = new WeatherValue(coordinate, directIrradiation, diffuseIrradiation, temperature, windDirection, windVelocity);
