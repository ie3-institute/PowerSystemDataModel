...
public class InfluxDbWeatherSource implements WeatherSource {
    ...
    public Map<Point, IndividualTimeSeries<WeatherValue>> getWeather(
        ...
        try (InfluxDB session = connector.getSession()) {
            ...
            Set<TimeBasedValue<WeatherValue>> timeBasedValues = filterEmptyOptionals(optValues).collect(Collectors.toSet());
            Map<Point, Set<TimeBasedValue<WeatherValue>>> coordinateToValues = timeBasedValues.stream()
                    .collect(Collectors.groupingBy(timeBasedWeatherValue -> timeBasedWeatherValue.getValue().getCoordinate(), Collectors.toSet()));
            return coordinateToValues.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, e -> new IndividualTimeSeries<>(null, e.getValue())));
        }
    }
    ...
    private Stream<Optional<TimeBasedValue>> optTimeBasedValueStream(QueryResult queryResult) {
        Map<String, Set<Map<String, String>>> measurementsMap = InfluxDbConnector.parseQueryResult(queryResult, "weather");
        return measurementsMap.get("weather").stream().map(fields -> new TimeBasedEntryData(fields, WeatherValue.class)).map(timeBasedEntryFactory::getEntity);
    }
    ...
    protected <T extends TimeBasedValue> Stream<T> filterEmptyOptionals(Stream<Optional<T>> elements) {
