...
public class CsvCoordinateSource extends CsvDataSource implements CoordinateSource {
    ...
    public CsvCoordinateSource(String csvSep, String folderPath, FileNamingStrategy fileNamingStrategy) {
    ...
    public void setupCoordinateMaps() {
        ...
        Stream<Map<String, String>> fieldsToAttributes = buildStreamWithFieldsToAttributesMap(fileName.get(), connector);
    ...
    private HashMap<Integer, Point> buildIdToCoordinateMap(Stream<Map<String, String>> fieldsToAttributes) {
        HashMap<Integer, Point> coordinateMap = new HashMap<>();

        fieldsToAttributes.forEach(map -> {
            try {
    ...
    /**
     * Tries to open a file reader from the connector based on the provided file name, reads the
     ...
     */
    private Stream<Map<String, String>> buildStreamWithFieldsToAttributesMap(String filename, CsvFileConnector connector) {
        try (BufferedReader reader = connector.initReader(filename)) {
            ...
            log.error(
                    "Cannot read file with name '{}': {}", filename, e.getMessage());
        }
