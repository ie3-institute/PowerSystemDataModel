 ...
 */
public class CsvWindowedTimeSeriesSource<V extends Value> extends TimeSeriesSource<V> implements AutoCloseable {
    ...
    public List<ZonedDateTime> getAvailableTimeSteps(String timeField, String timePattern) {
        ...
        try (BufferedReader reader = dataSource.connector.initReader(this.filePath)) {
            return dataSource.buildStreamWithFieldsToAttributesMap(TimeBasedValue.class, reader)
                    .map(
                            fieldToValue ->
